...
- [ ] ⬜ KV cache quantization — `LayerKv` stores `lc`/`rc` as **f32**. At GLM-5.2 shapes that is `(512 + 64) × 4 B × 78 layers` = **175.5 KiB/token ≈ 180 MB per 1,000 tokens**; int8 would be ~45 MB. MLA already compresses ~50× vs naive MHA, so precision is the remaining KV lever. Note `todo.md`'s own closed compression item says "skip unless the payload stops being quantized" — the KV is exactly the payload that never was. **f32 → f16 comes first and is nearly free**: every KV-quantization result in the literature is measured against an fp16 baseline, so peregrine starts 2× behind before any technique is applied. The reader seam (`KvSpan`) exists now; what f16 still needs is a materialization step, because both cores consume rows as `&[f32]` — decoding a whole layer's span into one reused scratch keeps the resident halving while the transient stays one layer's worth `attention.rs`
- [ ] ⬜ Paged / block-pooled KV — per-sequence contiguous `Vec`s grown by `extend_from_slice`, 78 per sequence, all long-lived and independently doubling. Acknowledged once in a doc comment at `model.rs` ("a paged/block-pooled variant that bounds many-sequence RAM is a follow-up") and tracked nowhere until now. Two of the three things this item was carrying have since landed separately: the unbounded ~53 GB worst case is now bounded by `COLI_KV_BUDGET_MB`, and cross-sequence sharing by the `Arc` prefix. What remains is genuinely fragmentation — `Vec` doubling overshoots capacity by up to 2× on a growing sequence, and a freed sequence's 78 allocations return to the allocator rather than to a pool the next admission can use. **The reclaimable waste is ~33%, not vLLM's 62–80%**: peregrine never pre-allocates (`LayerKv::new` starts at `Vec::new()`), so it has zero reservation waste — the largest bar in that figure. `KvSpan::row`/`runs` generalize to a block list without touching a reader `model.rs`, `attention.rs`
