...
| [Tools](tools.md) | `peregrine-gen` (watch and time generation), `peregrine-requantize` (fewer bytes per expert), `peregrine-skipbound` |
...
| [Measurement discipline](measurement.md) | how to get a number that means something here: medians over single runs, duty cycles over thread-summed counters, the page-cache trap |
