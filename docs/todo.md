...
[prefetch-and-caching.md](prefetch-and-caching.md#borrowed-negative-results) — they close off
...
*2026-08-21 wave (+3 open, all from outside review): the peregrine write-up was posted for
critique and `orionzion` returned the measurement design §13's cross-expert factorization item
was missing. All three additions are in §13 and none is started. The load-bearing correction is
that **weight-space reconstruction error is the wrong objective** — a basis can lower Frobenius
error while making the residual high-entropy and hostile to the int4/block quantization it then
has to survive, so the fit must be scored as rate-distortion **on activations**, under the
existing flip-rate gate, with the resident basis charged once. It also supplied the control
(**shuffled experts at equal rank**), without which a layer-wide-structure win would read as a
cross-expert-redundancy win — the same class of error as the domain gate in §4 and the weight
bound in `peregrine-skipbound`.*

*2026-08-21, later the same day: the first two shipped as `peregrine-basisfit`
(17 tests, workspace green, audit clean). **Running it found three reporting
defects that reading it had not**, all now branches with tests: a baseline at
the container's own precision is lossless by construction and its distortion is
a structural zero, so every ratio against it is meaningless; an arm can stream
more than the container ships and still win the arm-vs-arm comparison; and an
all-zero calibration vector makes every weighted error zero, which is a broken
sidecar rather than a perfect fit. **The control then caught a false positive in
the tool hosting it** — against a single shuffled draw the demo container read
"load-bearing" at 6.87 %, and against the best of four it reads 2.56 % against a
4.63 % spread, which is a negative. The real-checkpoint sweep is still owed.*

*2026-08-21, second round (+5 open across §1, §4, §7, §13): the write-up drew
technical review from four more agents and the useful part was corrective rather
than confirmatory. `cwahq` argued the **byte ledger should have come before the
factorization** — 11.3 GB/token is at least five numbers (requested, unique after
batch union, cache-served, missed-prefetch, re-read after eviction) and
`basisfit` prices against the *requested* denominator, which overcounts; the
standing rule from that thread is **flip-rate beside every saved byte**.
`miacollective` found the charging hole in `basisfit` — the basis is charged
**once**, which only holds while it stays resident, and at a 0.6 % warm-cache hit
rate it will not — and supplied both the degraded-predictor control for §1 and
the tolerance-in-the-key formulation for Q4. `lobbyagent` asked for a **p99 and a
page-fault/device split** that no counter in this repo can produce, which a mean
is precisely the statistic to hide; their page-cache and TLB claims do not
survive the code (`COLI_DIRECT` measures −23 %, and nothing `mmap`s the
checkpoint), but `miacollective` corrected the alignment half of it into
something real: target the SSD's **~128 KB internal page geometry**, not the host
page size, and the quantity protected is queue depth rather than TLB pressure.*

*2026-08-21, implementation pass: four of the six items above are built and
green (1006 tests, clippy clean on the new code, audit `--strict` OK). §1's
control arm, §4's latency histogram, §7's `align-cost` sizing, and the per-tick
basis charge in `basisfit`. **Running them found five defects that reading them
had not**, which is the pattern worth noting rather than the count: a `p99/mean`
tail ratio that reports a fat tail as flat because one outlier inflates the
mean; a `parent_device` that mangled `dm-0` and `md0` into names that probe
nothing; a verdict that printed the large-region explanation beside a 242 000 %
inflation figure; a basisfit report that quoted only the favourable basis
charge; and a test asserting 3 MB regions make alignment pointless that simply
**failed** — they straddle ~4 % for ~0.5 % disk. Two items remain open: cwahq's
...
requires io_uring: absent rings resolve to the `pread` engine for loading,
metadata and streaming alike (`streaming_runs_with_zero_rings` proves identical
logits with `rings=0`), page size is read from `sysconf` rather than assumed to
be 4096 (an aarch64 or ppc64le 64 KB-page host was reporting 1/16th of its true
RSS, so the guard never fired), and a prefetch pool that will not spawn is now an
advisory rather than a failed load. See [docs/portability.md](portability.md).

**aarch64 SIMD closed the same day.** `peregrine-kernels` had 13
`target_arch = "x86_64"` gates and none for aarch64; it now has NEON kernels for
int8, int4 and int2 — `sdot` where the CPU has ARMv8.2 dot-product,
`smull`/`sadalp` otherwise. `sdot` is emitted as inline `asm!` because
`vdotq_s32` is still nightly-only and this workspace is stable-only.

The blocker was never authorship, it was verification: the scalar path is the
token-exactness anchor and a disagreeing SIMD kernel fails *silently*.
`scripts/test-aarch64.sh` closes that — `peregrine-kernels` has zero
dependencies, so the musl target links with the toolchain's own `rust-lld` (no
cross C toolchain) and `qemu-aarch64` executes the equivalence suite as real ARM
instructions, with a guard asserting the dispatcher did not fall back to scalar.
16 tests, ~1s.

**Still owed: throughput on real ARM silicon.** Emulated wall-clock measures
nothing, so the kernels are known correct and reached, and their worth on a
Graviton or Ampere part is unmeasured.
...
[tokenizer.md](tokenizer.md#2026-08-13-speed-pass)).
...
`moe_engine_installed`, `pick_lfru`/`pick_swap`, `speculative_sample_at`, and
`WmmaTuner`'s `best_for`/`default_w4a16`/`default_int4tc`. *(Correction
2026-09-02: the sampled-draft accept path wired on the production track is
`speculative_sample_at` (`model.rs`); `mtp.rs::speculative_sample` is its
test-facing twin and remains test-only — an earlier revision named the wrong
sibling here and in the row at line 1058.)*
...
**Each now has a design** — [`docs/scale-out-design.md`](scale-out-design.md),
...
3. **Pure CPU work, nothing blocking but effort (8).** §12 fuse prefill into the decode batch, KV
   ...
   and it is now measured rather than inferred: the [2026-08-01 pass](benchmarks.md#benchmark-pass--2026-08-01-post-improvement-re-measure)
...
## 1. Prefetching & Speculation — 12/12 ✅
...
- [x] ✅ **A deliberately degraded predictor, to prove the scoreboard can see degradation** — `COLI_PREDICT_EVAL` now always carries a fifth arm, `control/noise`: deterministic uniform-random expert ids at the same emission rate as the real arms, seeded from `(layer, scored-layer count)` so a rerun reproduces it. **Not optional, and that is the point** — every other arm is one someone believed in, so "the instrument works" and "the instrument always says fine" produced identical output. Whatever the control scores is the **floor that comes free from guessing** (expectation `k/n`, small but not zero), and a real arm's recall only means something as a margin over it. `PredictEval::separation()` reports the best real arm against it and the engine prints that verdict **last**, so it is the line an operator leaves with; a scoreboard whose best arm is within 2× of noise says its recall columns should not be quoted until it is known whether the predictors degraded or the instrument cannot discriminate. Wired through the real forward path, not just the module's own tests — `predict_eval_scores_every_arm_over_a_decode` asserts the control is scored, never silent, and that `separation()` answers from a real decode. **Proposed by `miacollective`** (Moltbook, 2026-08-21). Their harder half is still open and stays in §1: scoring against realized routing measures what a predictor **allowed**, never what it **prevented**, and those diverge exactly when the fallback is good by construction — which here it always is `predeval.rs`, `model.rs`
...
production path reaches. Catalogued in [BAD_PATTERNS](BAD_PATTERNS.md#the-2026-08-02-triage).
...
- [x] ✅ **The expert map stops being inert: three defects that made routing data unreachable** (2026-08-20) — `TopicProfiles` is the only content-conditioned per-expert statistic in the tree, a `[5 TokenClass][n_layers × n_experts]` counter of "how often was this expert routed while the workload looked like Code/Json/Math/Prose". Under `peregrine-serve` it accumulated **nothing**, and the shutdown path wrote the empty profile out anyway — which is why a served model's `topic_profiles.json` reads as *learned nothing* rather than as *never ran*, and why `/home/cortix/models/GLM-5.2/route_stats.json` is 420 bytes with `coact.pairs: []` and `hist.layers: [78 × []]`. **(1) Accumulation.** `accumulate_topic()` had exactly one caller, inside `forward_hidden`; the batched forward sets `route_log: None` and never called it. Same for `CoActivation` (`model.rs:4541`). Both are now split into `accumulate_topic_for` / `accumulate_coactivation` and called per sequence from the engine tick, including the fused prefill chunk — where a request's content is most concentrated, so skipping it would bias the map toward whatever people ask *after* a long prompt. **(2) Attribution, which is the one that would have produced data rather than an error.** The key was the model's process-wide `workload_class`, set once per admission, so in a batch it held whatever was admitted most recently and a whole tick's routing got credited to one class. A code request's experts land under `Prose` because a prose request arrived after it, and the result *looks* like a topic map — nothing downstream can tell it is wrong. `Prefilling`/`SeqState` now carry the request's own class and each sequence's routing folds under its own key. `the_topic_map_accumulates_on_the_batched_path_and_credits_the_right_class` runs two concurrent requests with deliberately different classes, asserts both moved, and asserts the three classes nobody asked for stayed at **zero** — the assertion that fails under a single global key. Verified by simulating the old behaviour: it fails. **(3) Gate mass was unreachable from any artifact the engine produced.** `prune.rs::load_trace` claimed to accept `dump-routes` output and could not — that output is `[position][layer][expert_id]` and `f.get("layer")` on an array is `None`, so every element hit the `continue` and REAP exited *"no usable frames — nothing to rank"*; the pipeline in `docs/layout-tools.md` could never have run. `skipbound.rs` hit this exact defect, lost a run's numbers to it, and grew the branch on 2026-08-13; `prune.rs` never did. Ported, with the same regression test. And **no trace carried weights at all** — routing is recorded from `batch_union`, which keeps ids and discards `Routed.w` — so even parsed, saliency was counting wearing another name, exactly as prune's own report kept saying. `GateTrace` + `dump-routes --weights` emit the envelope form `{version,n_experts,frames:[{layer,experts,weights}]}`, captured where ids and weights still exist together; `read_routes` accepts both shapes and `read_routes_accepts_both_trace_shapes` pins that they project identically, so adding weights to a capture cannot move a number `route-stats` or `layout-reorg` already reported. Verified end to end: `peregrine-prune` now runs on the committed 256-position trace (19200 frames, 4 of 79 layers on aggregate saliency) where it previously errored, and a `--weights` capture yields real gate mass (0.361–2.139, post-`routed_scale`) rather than 1.0 placeholders. Seams `enable_topic_profiles` / `topic_heat_for` follow the `enable_calib_capture` precedent so tests never touch process env. **Worth knowing: routing is recorded on the streaming path only** — `route_log`/`route_log_multi` are written from `moe_forward_concurrent` and the resident `moe_forward` logs nothing, so a resident model has no routing history for topic profiles, co-activation or the predictor to fold `model.rs`, `concurrent.rs`, `batch.rs`, `prune.rs`, `tools/lib.rs`
- [x] ✅ **The expert map gate: measured, and it does not clear** (2026-08-21) — the question Phase 0 existed to make answerable was whether a *richer* map has anything in it: do different domains route to different expert cores? Four `dump-routes --weights` traces on GLM-5.2 (prose/code/json/techdoc, N=256, ~4.2 h), compared by top-K core Jaccard, within-label against across-label. **The answer is no, not by `TokenClass`.** Every pair beats the label-permuted null at z≈+3–4 and that number is worthless: routing has 34.7 % consecutive-position overlap against a 3.1 % null, so any contiguous grouping beats a shuffle — contiguous halves of a *single* prose trace read z=+11–19 with no domain effect present, which is why this measurement was rebuilt once. The right comparator is a same-`TokenClass` different-document control, and **`techdoc` classifies as `Code`, not `Prose`** (it is IT/cyber documentation, dense with paths and flags), which makes `code vs techdoc` the control and `prose vs techdoc` a genuine cross-class pair. Against that control **one cell of fifteen reaches 2σ**, uncorrected for fifteen comparisons, and the cross-class pair separates *no more than* the same-class control (−0.045/−0.011/+0.003 at K=100/600/2400) — the pair that should show a domain effect most clearly shows none. The apparent signal is document homogeneity: decomposing the pooled `within` values into per-document self-similarity is exact (6 equations, 4 unknowns, **max residual 0.0000**) and `json` is the most repetitive corpus at every K (0.395→0.412→0.446, the only one that *rises* with K); every pair showing excess involves it. **Phases 2 (`expert_map.json`) and 3 (output-neutral consumers) were not built**, per the plan's own gate. Recorded as a closed negative. What this does *not* close: `TokenClass` is a 5-bucket character-ratio heuristic over 512 chars — coarse enough to file technical documentation under `Code` — so this rules out *this key*, not the idea that experts specialise; a semantic key remains open. Phase 0a is still worth its own keep: it repaired a counter that recorded nothing under `peregrine-serve`. `bench-data/2026-08-20-expert-domains/README.md`, `ideas-tokens-per-sec-2026-08-15.md`
- [x] ✅ **Knob audit: every `COLI_*` read is documented, and two that were not fully implemented** (2026-08-20) — `configuration.md` calls itself "the complete env-var reference"; **19 knobs were missing from it**, ten documented nowhere at all — including `COLI_GPU_DENSE`, which `~/.config/peregrine/serve-qwen.sh` sets in *production*. All 19 now carry a row in their right section, and the two that **change token values** are in the quality-trade section and the `testing-and-quality.md` exceptions list, which said "Eight" and is now Ten: `COLI_GPU_DENSE` (the device path is *more* accurate — measured rms 1.1e-7 against the CPU path's 3.0e-3, because the CPU path quantizes activations to int8 — so *which* layers are resident changes the tokens, and any measurement arm must pin `COLI_GPU_DENSE_LAYERS` or two boots of one container differ) and `COLI_ACT_F32`. **Two real defects, both of the "parsed in more than one place" class:** (1) `COLI_NUMA_PIN` was read at **five sites with two polarities** — the policy sites opt-in (matching the documented default of off), the primitives `pin_current_thread`/`mbind_to_node` opt-out. Not a live bug, since each primitive has exactly one caller and that caller gates first, but the inner guards failed **open**: a future ungated caller would have pinned threads on a box that never asked. One `numa_pin_enabled()` predicate now, with `numa_primitives_are_inert_unless_the_knob_is_on` asserting the fail-closed property as a *consistency* claim so it needs no env mutation. (2) `COLI_IO_RINGS` — `io_rings()` clamps to 16 and the `[lane] io duty N% of R rings` shutdown line **re-read the raw environment without the clamp**, so `COLI_IO_RINGS=64` ran 16 rings and divided the duty by 64: a **4× understatement of the engine's headline occupancy figure**, in the direction that makes a saturated I/O lane look idle. The report now uses the resolved value; anything reporting against a knob has to resolve it the way the engine did. Also corrected: `physical-checkpoint.md` cited a `COLI_COACT_THRESHOLD` that never existed (the real one is `COLI_FUSE_THRESHOLD`, default 0.9, hyperedges at half of it), and `advanced-optimization-directions.md` now says outright that the env names it spells out are proposals no code reads `mem.rs`, `model.rs`, `batch.rs`, docs
- [x] ✅ **Open-loop arrival benchmark + `COLI_MTP_HEAT`** (2026-08-20) — the last two buildable items from the issue-#6 audit. **`scripts/bench-serve-arrivals.py`**: both existing serving clients are closed-loop, so the offered load is a *consequence* of the server's speed and the queue can never grow — the one regime where queue time is structurally invisible, which is why nobody had wanted the counter. This one submits on a Poisson process at a fixed `--rate` and never waits for completions, so the server's queue is the thing under test; sweeping the rate shows a knee. Measured on the tiny fixture: 50/s offered → 46.4/s achieved, queue mean **32 µs**, TTFT p95 1.17 ms; 400/s offered → 247.8/s achieved, queue mean **620 ms**, TTFT p95 1393 ms — i.e. **at saturation the queue is about half of TTFT**, and that half was invisible to every instrument this repo had. Running it immediately caught a trap worth recording: identical greedy prompts are answered from the **response memo** without touching the engine, so the first version offered 61 requests and the engine admitted *one* — `measurement.md`'s "benchmark that measures its own cache" in a new costume. Prompts vary per request by default now, `--repeat-prompt` restores the old behaviour for measuring the memo itself, and the script warns whenever admissions fall far below completions. **`COLI_MTP_HEAT`**: the draft `ForwardCtx` sets `heat: None` because a speculative draft must not feed a main-stream signal — correct in general and **over-broad for this layer**. The MTP head is layer index `n_layers` and *nothing except drafting executes it*, so its heat row has no main-stream competitor to skew; the table has been sized `n_layers + 1` since 2026-08-09 for exactly that row, and the blanket `None` has kept it empty ever since, leaving the LFRU eviction score and the VRAM reheat ranking blind to a whole layer — the same defect that resize was meant to fix. Off by default because it is a real trade (heat drives eviction and promotion, so MTP experts earning residency means main-stream experts losing it out of the same 12 GB) and because on a GPU build it changes which arm computes an expert `bench-serve-arrivals.py`, `model.rs`
- [x] ✅ **Queue time and joules per token** (2026-08-20, from the [issue #6](https://github.com/s-b-repo/peregrine/issues/6) audit) — two of that review's §20 measurement gaps were real, absent, and buildable without the checkpoint. **Queue time**: the audit asks for "request/s, latency, TTFT, p50/p95/p99, queue time" and TTFT plus inter-token percentiles already existed (`bench-serve-lanes.py`, `bench-serve-gaps.py`) — but every one of those instruments starts counting once a request is *already being served*, so admission latency was indistinguishable from slow decode, which on a server shedding at `COLI_QUEUE_DEPTH` is exactly the difference between "at capacity" and "over capacity". `/metrics` now reports `queue.{wait_us,admits,max_us,mean_us}`, counted per **admission** not per submit — a request refused at the door never waited, and averaging refusals in as zeros would flatter the mean precisely when the server is most overloaded. The timestamp is taken in `EngineHandle::submit`, the one place every request passes, rather than as a field on `EngineRequest`: a caller should not have to stamp its own clock to be measured, and one that stamped it wrong would bias the number silently. **Joules per token**: `EnergyMeter` existed but was read only under the opt-in `COLI_POWER_CAP_W` governor and never surfaced; `/metrics` now carries cumulative `energy_uj`, sampled on the governors' 16-forward period (totals stay exact — `delta_uj` reports everything since the last *call*, not the last tick). Two caveats that are the point rather than footnotes: RAPL covers **package-0 + core** on this box — no DRAM domain, no SSD — so on an engine whose bottleneck is 10.85 GB of expert reads per token the component doing the most work is the one RAPL cannot see, making this a *floor* on system energy; and `energy_uj` is root-only on current kernels (PLATYPUS), so it reads **null** unprivileged, deliberately not `0`, with the udev rule in `docs/external-audit-response.md`. Not built, deliberately: open-loop/continuous-arrival workloads (the counter is the prerequisite, not the study), long-context sweeps (need the checkpoint), same-hardware multi-engine comparison (a real gap, honest to leave open) `batch.rs`, `serve/main.rs`
- [x] ✅ **External audit triaged (`docs/external-audit-response.md`)** (2026-08-20) — issue #6's 21-section review is the most accurate outside description of this project ("a lot of optimizations have already been implemented, but many have not yet been demonstrated to improve real GLM-5.2 throughput") and its priority list **cannot be acted on as written**: it was assembled from documents predating several measurements, and at least two Tier 1 items are already shipped by default or already measured and rejected. Following it literally would spend scarce machine time re-deriving closed results — the exact failure `measurement.md` exists to prevent. Recorded per item with evidence: route pruning / min-share gating is **rejected** (τ=0.05 flips 27.9% of top-1 for ~12.5% read saving), prefill/decode fusion is **shipped and default on**, DSA is **wired on both paths**, int3 is **measured and failed twice** (0.514 / 0.447), f16 KV and block-pooled/paged KV and the byte-ceiling admission are **shipped**, and the single-run benchmark complaint is **fixed in the harness**. What the audit gets right and stays open: bytes/token as the fundamental constraint; **data-aware sub-4-bit (VQ / AQLM / QuIP#) as the strongest open idea** — every failure on this ladder used data-*free* RTN, which is not evidence against codebooks, and `--calib` + `calib-capture` are already code-complete for the importance-weighted rung; multi-GPU / GPUDirect / distributed as hardware-gated rather than undesigned; and a fresh controlled end-to-end benchmark as the single most valuable missing piece
- [x] ✅ **Decode economics are measurable: `decode.tokens_emitted` / `decode.rows`** (2026-08-20) — §6 and `docs/measurement.md` both say to judge speculation and fusion on counters rather than wall clock, and then the counter that would settle it did not exist. `/metrics` published the **denominator** all along (`ecache.hits + misses` is every routed-expert entry the streaming lane resolved; `disk_reads` the subset that reached the device) and nothing said what those reads bought. Two cumulative numbers close it, with **no hot-path change at all**: `Δtokens / Δ(hits+misses)` is tokens per expert read, `Δtokens / Δdisk_reads` is tokens per disk read — `speculative-decoding-alternatives.md`'s stated metric, SSD bytes per *accepted* token, measured instead of derived — and `Δrows / Δtokens` is speculation's row overhead, the quantity an expert-union budget would be spent against. **Exactness is the whole value and the test is what enforces it**: the first version counted only the decode loop and missed the token each request emits at *promotion* (sampled from the prefill's last position), which understates tokens-per-read by ~20% on short requests — a bias in the flattering direction, exactly the shape of error this repo has had to walk back four times. `finish_prefill_chunk` and `prefill_step` now report what they emitted. Two consequences worth knowing before reading the ratio: a request's first token costs **no** decode row, so `rows/token` sits just *below* 1.0 unspeculated and approaches it as requests lengthen (above 1.0 is speculation's overhead, and a short-request workload reading 0.8 is not a win); and telemetry publishes at tick *end*, so a client can hold the last tick's tokens before the snapshot lands — `decode_counters_are_the_numerator_the_ratio_needs` asserts the tick-boundary invariant plus an upper bound rather than an equality that passes alone and fails in a loaded suite. **This is deliberately the instrument and not the gate.** `COLI_SPEC_UNION_MAX` — dropping the deepest draft rows when projected union growth exceeds a ceiling, the cost-side twin of `COLI_SPEC_CONF`'s acceptance-side floor — stays specified and unbuilt until these counters say whether speculative rows are already union-cheap under the 0.65 floor. Building a gate tuned against an unmeasured quantity is the failure `measurement.md` opens with `batch.rs`, `serve/main.rs`
- [x] ✅ **`peregrine-requantize --mtp-target`** (2026-08-20) — the MTP head's expert pool is the last **int8** rung in a GLM-5.2 container (37,748,736 bytes/expert against 18,874,368 at int4) and it is read in the worst regime in the engine: once per *draft step*, at `s_n = 1`, with no batch-union amortization — ~300 MB of SSD per step at topk=8. Halving it halves the draft path's bytes, and **it is the only point on this ladder that needs no flip-rate gate**: a draft is accepted only where it equals the verify forward's own argmax, so a coarser draft head can move the acceptance rate and cannot move a served token. (Run the gate anyway, as an *assertion* that it reads 0.000 — that is what proves the override landed on the head layer and nowhere else.) Deliberately the most specific selector, beating both `--keep-last-layers` (whose window counts `n_layers + 1` slots and so already covers the head) and `--down`, because a flag naming one layer is a statement about that whole layer; `mtp_target_hits_the_head_layer_and_beats_the_keep_window` pins all three precedences plus the no-override case, since a flag that looks applied and does nothing is the failure mode here. Policy went into `plan_target`, the one function whose doc comment says every future policy goes there or it repeats the `--dry-run --tier-hot-frac` drift. **Needs a conversion run on the real container to become a number** — this workspace cannot load it `requant.rs`, `requant_main.rs`
- [x] ✅ **Pin the MTP layer's hot experts** (2026-08-22) — `mtp::MtpPins` + `mtp::plan_pins`, `COLI_MTP_PIN_MB` (warm cache) and `COLI_MTP_PIN_VRAM_MB` (VRAM), `WarmCache::{PIN_PRIORITY, clear_pins}`, `GpuTier::sync_pins` + `gpu::PinRequest`. **The reason this was never a flipped flag turned out to be one line wider than this entry originally guessed.** It is true that the draft `ForwardCtx` withholds `heat` deliberately, and `COLI_MTP_HEAT` (2026-08-20) already un-withholds it. But filling that row buys no residency, because **every residency planner in the engine enumerates candidates over `first_dense..n_layers`, exclusive** — `plan_residency`, `rank_by_heat`, `solve_residency_sized`, `plan_precision_fitted`, `plan_swaps`, and `Model::protect_from` — and the MTP head is layer `n_layers`. The `HeatTable` was widened to `n_layers + 1` on 2026-08-09 so that row would exist; nothing has ever proposed a candidate out of it. And `Model::heat` is `gpu.as_ref().map(..)`, so on the CPU-only streaming deployment — the one regime where a draft step costs ~300 MB of SSD at `s_n = 1` with no batch-union amortization — there is no table to write to at all. **A reservation, not a rank**, and that is the whole design: everything else in these tiers competes for one budget on one score, and this layer cannot win that comparison honestly — it is read once per *draft step* against 78 layers read once per *token*, so any frequency-ordered policy ranks it last however much each of its reads costs, while the main stack's own sweep evicts it between every pair of draft steps. So the pin's bytes come off the top: `reheat` solves the heat plan against `budget − pinned_bytes`, and the warm cache gets a priority above anything `pack_prio` can now produce (clamped to `PIN_PRIORITY − 1`, so a maximally-scored prediction can no longer *tie* a pin and fall through to recency — which is the ordering a pin exists to remove). The VRAM arm has to **make** that room before it uploads, in heat order (`evict_coldest_for`), and that is not a refinement: `build` fills the budget, so on a working tier the first generation with a reservation set finds no free VRAM, every pin upload fails, and — since each generation retries identically — nothing ever lands and the log fills with advisories. Evicting here rather than leaving it to the re-plan is also what makes the reservation work under the incremental swap policies, which hold the resident *set size* fixed and so can never free room for one on their own; the victims are the coldest non-pinned residents, i.e. the same ones the re-plan was about to drop, so it is that eviction moved early rather than an extra one. **Chosen by drafting, and empty until drafting has said something**: a private per-expert counter for that one layer, bumped at the same site `heat` is and refusing any layer but its own, re-planned every 64 draft rounds; experts with zero observed routing are never pinned, because a cold table says nothing about which eighth of a 256-expert pool matters and ranking it would spend the entire budget on ids `0..K`. `rank_by_heat` makes the opposite choice for a good reason — at load *something* must be resident — and a pin carries no such obligation. **Budgets in bytes, not expert counts**, since the pool is ~4.8 GB at int4 and ~9.7 GB at the int8 rung this container still ships, and each is clamped to half its tier with a one-shot line saying so: neither evictor refuses to evict a pin, so a mostly-pinned tier starves everything else and then evicts among the pins on recency anyway. Two smaller decisions fell out of the wiring. The refresh is driven from the **draft path**, not `Model::reheat`, because `reheat` has exactly one caller (`peregrine-serve`'s batched engine) and riding it would have left the CLI speculative path permanently unpinned — the same "mechanism exists, nothing calls it" shape this item was. And the counter is **not** gated on the budgets, because a load-time env latch decides the arm before an A/B can measure it; the spending is gated, the counting is eight relaxed `fetch_add`s on a step that streams 300 MB. Pinned by `drafting_feeds_the_mtp_pin_table_and_pins_what_it_routed` — forced-streaming tiny model, asserting that a main-stack forward leaves the table empty, that drafting fills it, that the plan is *exactly* the routed set, and that it lands as `PIN_PRIORITY` on that layer and no other — plus `a_pin_outranks_the_highest_predictor_protection` and `a_pin_survives_a_sequence_reset_and_the_negative_cache` (a pin outlives `clear_priorities` because it is a per-process residency decision where predictor protection is per-stream; the negative-TTL sweep already skipped `prio != 0`). **Found while splitting `reheat`**: `reheat` → `reheat_incremental` → `reheat` was unbounded mutual recursion whenever `COLI_GPU_TIER_SWAP` named a swap policy *and* `COLI_GPU_F32_FRAC` was set — a stack overflow on the first residency generation, from two knobs each exercised alone and never together. The re-plan body is now `reheat_replan`, which is what that fallback calls. **Not measured on GLM-5.2 yet**, and the VRAM arm is worth setting only after the `--mtp-target int4` conversion run: an int8 expert cannot be int4-resident, so it uploads dequantized at ~151 MB against 18.9 MB — eight experts on a 12 GB card for what should hold sixty-four. `/metrics` carries `mtp_pinned.{cache,vram}`; read them against `ecache.disk_reads`, which is the only thing that can say whether the reservation bought the re-reads it was set to stop `mtp.rs`, `model.rs`, `gpu.rs`, `concurrent.rs`, `warmcache.rs`
- [x] 🟡 **The MTP draft path stops paying B disjoint unions** (2026-08-20) — `Model::mtp_draft_batched` + `expert_index` in the draft context. §6 recorded speculation's cost as union growth on the *verify* path; the exploration found the larger hole is on the **draft** path, and that it was never a design decision. The MTP head is a sparse MoE layer with its own expert pool at layer index `n_layers`, stored **int8** — 37,748,736 bytes per expert against 18,874,368 on a normal int4 layer — and `mtp_draft` ran it at `s_n = 1`, once per sequence per step. So B sequences drafting γ deep streamed **B·γ disjoint routed-expert unions** (~300 MB of SSD per step at topk=8, ≈1.2 GB per round at γ=4) before the verify forward, which shares one union across `B·(1+γ)` rows, had even started. `mtp_draft_batched` runs step `g` for every still-drafting sequence as one row of one call, each on its own local `LayerKv` at position `g`, so the batch-union covers them exactly as it covers concurrent decode rows: **B unions per step become 1**. Depths are per sequence (`g_of[i]`), because a nearly-finished request drafts less and drafting it deep then truncating would spend real ~300 MB steps on discarded tokens. Greedy only — the sampled path needs each sequence's own `Sampler` and the `q` it drew from, so `COLI_DRAFT_SAMPLED` keeps the per-sequence loop. **Asserted as token equality, not a tolerance**: `batched_drafting_proposes_exactly_what_per_sequence_drafting_does` checks three ragged sequences at two confidence floors, plus a hole in the middle (`hlast` empty) and ragged depths, against drafting each alone — a differing draft would silently move the acceptance rate and read as a tuning result. Separately, `expert_index` was the one `None` in the draft `ForwardCtx` with **no comment explaining it**, and that was the tell: every other withheld field is a main-stream signal a draft must not feed (heat, prediction, calibration, lane balance), while the expert index is just the load-time `(layer, expert) → (fd, offset)` map. Without it every draft step resolved through `entry_for`'s `format!` fallback *and* lost the `(fd, offset)` sort, issuing reads in routing order on the one path in the engine with no batch-union amortization to hide it. Correctness-neutral: same expert either way, and completion order cannot reach a position-keyed reduce. **Still open on this path**: the MTP layer's experts are the only int8 rung left in the container and requantizing that one layer to int4 halves the draft path's bytes **with no flip gate needed** — a draft is accepted only where it equals the verify forward's own argmax, so degrading the draft head can move the acceptance rate but cannot change a served token. (Pinning that layer's hot experts is done — the entry above, 2026-08-22.) `model.rs`, `batch.rs`
- [x] ✅ **`COLI_DRAFT_TREE`: the two draft sources stop being alternatives** (2026-08-20) — prompt-lookup and the MTP head are frequently right about *different* continuations, and until now the n-gram simply won and the head's chain was discarded unseen. One forward can verify both: root = the pending token, one branch per source, and whichever the model's own argmax follows commits by the same greedy-identity rule a chain uses. Skipped when the two agree on their first token — the branches would not be alternatives and two rows to verify one candidate is strictly worse than the longer chain. **The one thing a chain never needed is the commit.** An accepted path is a *non-contiguous* subset of the block's slots with rejected siblings interleaved, so `truncate` cannot express it; `LayerKv::retain_tail` gathers the kept rows down instead, and that is a **pure move** rather than a recomputation for two reasons worth stating: latents do not depend on position, and each row was roped at its **tree depth**, which is exactly the position it lands on once its ancestors pack below it. `committing_a_tree_path_leaves_the_cache_a_plain_chain` asserts that end to end against decoding the same branch as an ordinary chain from the start, *and* requires that a plain `truncate` would NOT match — otherwise it would pass with the gather never running. `retain_tail` refuses a `from` inside a shared prefix (immutable, other sequences reading it), a descending keep-list and an out-of-range one, because all three corrupt silently. **Mixed batches keep the fast path**: `sel` became per-row `Option`, where `None` means dense, so one sequence hedging never drags the rest of the batch onto the explicit-key loops — and the DSA arm now returns `None` below `index_topk` instead of materializing an identity set, which is a small free win it was leaving on the table. Counters `spec.trees` / `spec.tree_branch_wins` say whether the hedge is paying; trees climbing with branch_wins flat means the extra rows buy nothing. **Two costs written down rather than discovered**: every branch row is a full row of the routed-expert union (bytes, not FLOPs, on the streaming container), and a tree row's key set is explicit so it is O(context) to build — making trees cheapest at *short* context, the opposite of the usual intuition, until a compact `prefix + extras` representation exists. MLA-only, greedy-only `batch.rs`, `attention.rs`, `model.rs`, `tree.rs`
- [x] ✅ **Token-tree verification substrate** (2026-08-20) — `tree.rs` (`CandidateTree`, `accept_tree`, `TreeRows`), `RowLayout::{rope_pos, sel}`, `Model::forward_tree_rows` (production runs the `_hidden` variant; the un-suffixed name is the test-facing twin). The shared substrate under tree speculative decoding, token-tree execution, blockwise parallel decoding and EAGLE-2's dynamic tree — those differ in how a tree is *built*, not in how it is run. **`LayerKv` did not have to change**, which was the surprise: lay the candidates out in DFS order as ordinary consecutive slots and the cache never sees a tree, so the strictly-ascending `append` invariant, the `Arc` prefix sharing, the DSA key stream and the speculative rewind all keep working. What makes them a tree is two vectors the row layout now carries — `sel` gives each row its ancestors (siblings are absent from each other's key sets) and `rope_pos` gives each row its tree *depth*, so siblings share a logical position instead of being rotated as a sequence. `sel` already existed for DSA in both cores; a caller-supplied mask replaces the DSA one rather than intersecting, because a DSA mask is an efficiency choice and a tree mask is a correctness constraint that must not drop an ancestor. **Proven, not argued**: `a_linear_tree_is_bit_identical_to_the_chain_path` pins that the new plumbing moves nothing on the shape the engine already runs; `a_tree_row_cannot_see_its_siblings` checks a branch against the same branch run alone on a real forward *and* requires that an unmasked third row would differ, so it cannot pass with `sel` ignored; `accept_tree_reduces_to_accept_run_on_a_chain` sweeps draft depths × peak patterns. **The constraint that inverts the expected value, found while building it: trees are MLA-only.** A recurrent layer advances one delta-rule state row by row, so siblings would chain rather than branch (per-branch state = ~3.1 MB × 48 layers), and the batched GQA path takes no key set so a mask would be silently ignored — `forward_tree_rows` refuses both loudly. So trees work on the *streaming* track where extra rows cost disk bytes, and not on the *resident* track where they would be nearly free, which is why tree width has to be spent against a union-cost gate rather than set to a constant. The `batch.rs` half landed the same day as `COLI_DRAFT_TREE` — see the entry above it `tree.rs`, `attention.rs`, `model.rs`
- [x] ✅ **Prompt-lookup drafting (`COLI_DRAFT_NGRAM`)** (2026-08-20) — a *second* draft source feeding the verify path that already exists, costing no weights, no forward pass and no training: it proposes whatever followed the most recent earlier occurrence of the sequence's current suffix (`ngram.rs`). The reason it is worth a source rather than a knob is the cost asymmetry — an MTP draft step is a full sparse-MoE layer at `s_n = 1`, ~300 MB of SSD per step on the streaming container with none of the verify forward's batch-union amortization, and this is a backward `memcmp` over the token log. `ideas-from-colibri.md` had already written the reason it matters *here* specifically: on a disk-bound engine speculation only loses when drafts are **rejected**, so a source whose acceptance approaches 1 sidesteps the failure mode both engines measured. It takes priority over the head whenever it matches, and the two are alternatives per tick rather than a chain because `mtp_draft` continues from a hidden that assumes its own prefix. **Greedy only, and not by policy**: an n-gram draft is drawn from no distribution, so `accept_run_sampled` would have no `q` and the distribution-preserving guarantee would be void — `COLI_DRAFT_SAMPLED` does not extend to it. **It needs `COLI_DRAFT` for the depth but not an MTP head**, which is a capability gain: `prompt_lookup_speculates_without_an_mtp_head` shows a container converted without `--mtp` speculating, with `spec_proposed == 0` asserted so it cannot be the head doing it. Deliberately never *invents* a continuation, only replays what history holds, so a run of identical tokens drafts one per tick instead of filling the depth — on the streaming track an invented token that misses costs a verify row of expert reads. Counters live in their own `ngram` block on `/metrics` rather than pooled into `spec`, because one accept rate averaged over a free source and an expensive one decides nothing. Writing the tests surfaced the alignment trap worth recording: `draft_depth_for` clamps depth to `budget_left - 1`, so the last positions of a short run never draft, and an 8-token test spent its only suffix match there and read as "the drafter never fired" `ngram.rs`, `batch.rs`
- [x] ✅ **Recurrent speculation (`COLI_SPEC_GDN`)** (2026-08-20) — the MTP head above was wired for *both binaries* but only for **one architecture**. `spec_reject_is_kv_only()` returned false for `Arch::HybridGdn`, so `peregrine-serve` refused to draft on Qwen3.5 and said so on stderr: a linear-attention layer keeps a delta-rule state, not rows, and `truncate` cannot take a rejected draft back out of it. That excluded speculation from the **resident** track — `serve-qwen.sh` on :8132, `COLI_STREAM=0` — which is precisely the regime where speculation's economics are textbook-favourable, because a verify row there costs compute on weights already in RAM instead of the 2.63× expert-union growth measured on GLM. The rollback primitives (`SeqKv::gdn_snapshot`/`gdn_restore`, `GdnState::snapshot`/`restore`) had shipped in full, with the protocol written into their doc comments, and had **zero callers**. Now: snapshot before the verify forward for every drafting sequence, drop it untouched on full acceptance, and on partial acceptance restore → rewind the KV to the same boundary → re-advance over exactly the committed rows, with every partially-accepting sequence of a tick re-advancing in **one** forward rather than one each. **Output-neutral and asserted three ways, each with a negative control**, because a rollback test that passes when the rollback does nothing is testing nothing: `a_rejected_draft_leaves_no_recurrent_trace` (k=0) also requires that a bare `truncate` does *not* match, `a_partially_accepted_draft_re_advances_exactly_the_committed_rows` (k=1) also requires that replaying one row too few does not match, and `recurrent_speculation_does_not_change_the_served_token_stream` runs the real engine against an independent greedy decode and requires `spec_proposed > 0` and `gdn_snapshot_bytes > 0` so it cannot pass by not speculating. Needed a fixture: `build_tiny_hybrid_model_with_mtp` emits the Qwen-dialect head (`mtp.fc.weight`, `mtp.layers.0.*`, zero-centered norms), kept as a **separate** builder on the `build_tiny_model_with_indexer` precedent so the existing hybrid tests keep loading a checkpoint where `has_mtp()` is false. **The cost is real and is reported rather than assumed**: ~3.1 MB per linear layer, ≈151 MB per drafting sequence per tick at 27B dims, charged per *sequence* while a forward's resident weight read is shared across the batch — hence `COLI_SPEC_GDN_MAX_B` and the `gdn_snapshot_bytes`/`gdn_replays`/`gdn_replay_rows` counters on `/metrics`. Note for whoever tunes it: full acceptance is the free path, and at a per-token accept rate of 0.83 a depth-5 run completes only 39% of the time, so replays are the common case until the `COLI_SPEC_CONF` floor is tuned — that floor, not the snapshot, is the lever `batch.rs`, `model.rs`, `gdn.rs`, `testkit.rs`
- [x] ✅ **DSA reaches the batched attention core** (2026-08-20) — `sel`, the per-row explicit allowed-key set, was threaded through the dense core only, so the *served batch path* ran long context dense while the single-sequence path selected; `forward_prefill_seq` takes `forward_layer`, which is why `dsa_selects_a_subset_once_context_exceeds_index_topk` never covered it. `attend_absorb_batched` now takes `sel` too, `mla_attention_rows` builds the selection (index keys ride each row's own `LayerKv` as the third stream they already are), and `forward_layer_batched` chooses it with the same `ctx.dsa.then_some(()).and(l.indexer.as_ref())` expression the single-sequence path uses. `mla_attention_batched` gained the same parameter rather than hard-coding `None`, since it is documented as that function's one-token-per-sequence case. The new arm memoizes materialized indexer keys **per owner**, which is exactly where a cross-sequence bug would live — score B's query against A's keys and the output is wrong but plausible — so `batched_dsa_rows_get_what_each_sequence_would_get_alone` puts two sequences through one forward and requires each to be bit-identical to running it alone, measuring the dense arm first so it cannot pass with selection inert `attention.rs`, `model.rs`
...
- [x] ✅ **Per-read latency distribution — p99, and the page-fault/device split** (`COLI_IO_LATENCY`) — every I/O figure this repo published was a mean or a steady-state aggregate, so the engine could not answer "what is p99 for one token, and how much is page-fault handling versus device time". `peregrine-io/src/latency.rs` is a log-scale histogram (4 sub-buckets per octave, 96 buckets, 0 µs–16 s) plus a `RUSAGE_THREAD` minor/major fault delta over the same window, wired per **completion** in `Reactor::read_many` — per request, not per wave, because a wave mean smooths away the single slow read being hunted. **Deliberately a histogram and not an EWMA**: nothing here decays, because decay is the defect — `IoTuner` smooths exactly the signal a GC stall would show up in. Off by default; a tail hunt is a diagnostic run.

  **Two design errors were found by the tests rather than by review.** The first `tail_ratio` was **p99/mean**, which is wrong in the only case that matters: a single large outlier drags the mean *above* p99, so a fat tail reported as a ratio below 1 and read as flat. It is p99/**p50** now — the median is not moved by the outliers being hunted. And p99 structurally cannot resolve fewer than `count/100` stalls, so one stall in a hundred reads reports flat and correctly so; `max/p50` is tracked beside it and the report distinguishes "no tail" from "rare stalls this window cannot resolve". Undersampled quantiles print `n/a(20<100)` rather than a number, because a p99 from twenty samples is the slowest of twenty wearing the name. The report also refuses to imply a device measurement: submit→complete contains queueing behind the ring depth cap, and says so. **Raised by `lobbyagent`** (Moltbook, 2026-08-21); their stronger claims did not survive the code (`COLI_DIRECT` measures −23 %, and nothing `mmap`s the checkpoint) but the tail question was unanswered `latency.rs`, `ring.rs`
...
## 7. Disk Layout & Offline Optimization — 11/11 ✅
...
- [x] ✅ **Align relocated expert regions to the device's block granularity — *sized*, and the writer deliberately untouched** — `peregrine align-cost <model-dir>` prices the trade the item was gated on. `peregrine-io/src/geometry.rs` probes the drive's own `optimal_io_size` from sysfs (resolving partitions to their parent device) and models, over a container's real routed-expert offsets, the device units touched as laid out versus with every start on a boundary, against the padding bytes that would cost. **The constant is probed, not chosen**: an unprobeable device reports `ASSUMED` in the verdict rather than quoting the 128 KB fallback as a measurement — quoting an unprobed constant is precisely how the host-page-size version of this idea went wrong.

  **Three defects, all found by running it.** `parent_device` stripped trailing digits generally, so `dm-0` became `dm-` and `md0` (a RAID array, not a partition of `md`) became `md` — both would then probe nothing and fall back silently, indistinguishable from a device that advertises none; it is an explicit `sd`/`hd`/`vd`/`xvd` allowlist now, failing in the safe direction. Regions smaller than the unit and regions larger than it both report "no units saved" for **opposite** reasons, and the first version printed the large-region explanation for both — which on a fixture of 53-byte tensors read as reassurance beside a 242 000 % inflation figure. And a test asserting that ~3 MB expert regions make alignment pointless **failed**: they span ~23 units and starting mid-unit adds one, about 4 % more units for about 0.5 % more disk, so the model calls it worth measuring for real. That corrected expectation is recorded rather than tuned away.

  **What is still owed: the run on the real shards.** A unit count is not a latency, and `iobench` with a cold cache is what would turn one into the other. **Raised by `lobbyagent`** (Moltbook, 2026-08-21) as 2 MB filesystem hugepages and **corrected in the same thread by `miacollective`**, who had run it: the TLB win was swallowed by allocator fragmentation and what paid was aligning to the SSD's ~128 KB internal geometry, protecting **queue depth** rather than TLB pressure `geometry.rs`, `engine/main.rs`
...
- [x] ✅ Mixed-precision execution per expert — `plan_precision_fitted` promotes the hottest `COLI_GPU_F32_FRAC` of residents to f32, rest int4 (pure, unit-tested); wired into `real::GpuTier::reheat` with per-expert format tracking + re-upload-on-change (type-checked under `--features cuda`). *Correction 2026-09-02: an earlier revision named `plan_precision` here — that is the unsized predecessor whose wiring reintroduced the 542→67 residency bug §7 records; the wired function is the fitted one.* `gpu.rs`
...
- [x] ✅ **sPTC — speculative programmatic tool calling (2026-09-02)** — a tool call streams one token at a time; with `--host-tool` the server now runs *hosted* tools behind the stream instead of always handing the finished call to the client: complete argument pairs are parsed incrementally out of the still-open `<tool_call>` body (`tools.rs::parse_call_prefix` — unterminated values excluded, so a snapshot's arguments are a **prefix** of the final ones), a bounded shadow pool (2 workers, 16-deep queue, skip-not-block) executes them, and close-time verification looks the final call up by **exact canonical key** (tool name + sorted-key JSON through the same schema-typed `coerce`; a hit attaches `peregrine_result` to the existing `tool_calls` chunk, a miss runs synchronously and deposits its result for the next identical call). Correctness rests on one structural fact: the `pure_tool!` decorator is the *only* way to build a `HostedTool` (the run closure is private), so "deterministic, side-effect-free, safe to run on a partial guess" is a property of the type — a wrong guess wastes CPU and nothing else, the same correctness-neutral shape as the prefetch lane. The closed built-in set is tokenizer queries (`tokenize`/`detokenize`/`count_tokens` — pure, in-process, no ambient authority: no disk, no network, no engine state), an unknown `--host-tool` name is a boot error, `COLI_SPTC=0` kills speculation while keeping execution, and with nothing hosted the subsystem does not exist — no threads, no table, no per-token work, byte-identical streams. Counters (`sptc.speculated` / `verified_hits` / `sync_runs` / `rejected_*`) live on `/metrics`; hits ≈ speculations with `sync_runs ≈ 0` means the shadow is saving its wait. Tested at every seam: prefix-vs-final key equality, mismatch-discard, dedup per new pair, impure-impossible, kill switch, input cap refusing before any run, table bounded on both axes with exact eviction counts, failure-caching, memoized-replay parity (a replayed streaming call attaches results exactly like a live one — a gap a subagent audit found and closed), and the unhosted pass-through that must not grow the field `sptc.rs`, `tools.rs`, `main.rs`, `docs/sptc.md`

---

## 13. Workload Reduction — 16/21
...
reachable 33.55% is real and the engine captures ~0.4% of it. And the [2026-08-01 benchmark pass](benchmarks.md#benchmark-pass--2026-08-01-post-improvement-re-measure)
...
- [ ] 🟡 **Cross-expert factorization measured as rate–distortion on *activations*, not weight reconstruction** — **harness shipped 2026-08-21 (`peregrine-basisfit`); the number on a real checkpoint is still owed.** Fits `W_e = B + Δ_e` per layer (mean plus `--rank` principal directions, via the Gram trick over the group — the weights are millions of elements and the group is tens of experts, so the covariance is never formed), holds `B` resident, quantizes the residual through the **same `quant_*` producer and `QtView` consumer the loader uses**, and scores the error weighted by the calibrated per-channel `mean|x|` from an existing `calib-capture` sidecar. Frobenius error is *not* the objective and the tool says so in its own `--help`: a basis can lower `‖W_e − (B + Δ_e)‖_F` while making `Δ_e` high-entropy and hostile to the int4/block quantization it then has to survive, and weight-space error is structurally blind to that because it never quantizes anything. **Both arms default to the same precision one rung below the container**, so streamed bytes are identical and the only variable is whether the residual quantizes better than the weight; the resident basis is charged separately in f32 and never folded into the comparison. **One charging assumption is unverified and `miacollective` flagged it** (Moltbook, 2026-08-21): the basis is charged **once**, which is only right if it stays resident. Under cache pressure an evicted basis is re-read per tick, and they report a rank sweep that "looked impressive until I charged the resident basis per tick, not once". With a 10 GB cache against a ~180 GB working set that is not a hypothetical here — the tool should report both charges, and until it does the resident column is a lower bound. Three reporting defects were found by *running* it rather than by reading it, and each is now a branch with a test: requantizing int4→int4 is lossless by construction, so a baseline at the container's own precision is a **structural zero** and every ratio against it is meaningless (refused, not scored); an arm can stream **more** than the container ships and still win the arm-vs-arm comparison, which is true and useless (the container comparison now runs first); and an all-zero calibration vector makes every weighted error zero, which is a broken sidecar rather than a flawless fit. Read amplification is measured against **the container as shipped**, not against the baseline arm, because the operator question is "fewer bytes than what I run today". **What remains: the sweep on real GLM-5.2 weights** — rank × residual-precision, and the flip-rate gate on whatever wins. Proposed by `orionzion` (Moltbook, 2026-08-21), whose framing this entry is `basis.rs`, `basis_main.rs`

- [x] ✅ **Shuffled-expert control for the basis fit** — implemented alongside the harness above, validated in **both** directions, and it has already caught a false positive in the tool that hosts it. `--control` runs the learned grouping against **`CONTROL_DRAWS` = 4 random partitions** at equal rank and equal residual precision, every arm through the same code path with only `Grouping` differing — a control implemented separately from the thing it controls would be comparing two implementations, not two groupings. **One draw was not enough, and finding that out changed the verdict.** The first version compared against a single shuffled partition and reported the demo container's grouping as "load-bearing" at a 6.87 % margin; against the best of four draws the same container reads 2.56 % against a 4.63 % spread of the draws themselves, and the verdict flips to negative. The floor is `max(15 %, spread)` rather than a sign test, because the learned arm is fit and scored on the same experts. **An assumption of mine was wrong here and the test corrected it**: I expected that selection bias to make the learned arm win even on structureless weights, and it does not — taking the *best* of several draws is itself a selection over the null, and empirically the two cancel. That cancellation is exactly why the comparison is against the best draw rather than one draw or the mean, and `the_control_does_not_fire_on_experts_with_no_structure` pins it; if it ever stops holding, every positive verdict the tool has emitted needs re-reading. Its twin `the_control_does_fire_when_the_grouping_is_real` runs two genuine clusters, without which the first test only proves the control is inert. Proposed by `orionzion` (Moltbook, 2026-08-21) `basis.rs`

- [x] ✅ **The byte ledger — 11.3 GB/token decomposed into the numbers it is actually made of** — `peregrine-model/src/ledger.rs`, printed at shutdown by `bench` and the serve path whenever `COLI_UNION_STATS=1`. Five columns: **requested** (`Σ keff` — the arithmetic figure everyone quotes), **unique after the batch union**, **cache-served**, **from disk**, and **prefetch waste**. Every column is bytes rather than slab counts, converted by the container's own per-expert size, and a tiered container marks its own figures `ESTIMATE` because `count × bytes_per_expert` stops being exact there. Denominators are chosen so as not to flatter: prefetch waste is a share of **disk traffic**, not of `requested` — the latter is ~4× larger and would report 2.3 % where the truth is 60 %.

  **The union column already existed and had never been assembled into a ledger**: `router::union_stats_snapshot` counts selections and distinct experts, so the batching claim's byte identity was one accessor away the whole time. What was missing was a denominator — `rows_forwarded`, added to **both** `forward_step` and `forward_step_batched`, because the first version counted only the single-sequence path and reported the ledger "over 0 tokens" on exactly the batched runs it exists for.

  **The rule is enforced structurally, not left to the reader**: the report refuses to print a saving without naming the flip-rate gate that has to qualify it, and carries the precedent — `COLI_ROUTE_MIN_SHARE` cut 12.5 % of reads for 27.9 % of top-1. One column is **NOT MEASURED** and says so rather than being folded silently into `from disk`: bytes re-read after a wrong eviction need per-slab eviction history the warm cache does not retain. **Proposed by `cwahq`** (Moltbook, 2026-08-21), who argued it should precede the factorization work; the argument was right, and `basisfit` prices against the requested denominator rather than the unique-after-union one `ledger.rs`, `model.rs`, `engine/main.rs`

- [ ] 🟡 **Token-equivalence memoization keys the tolerance, not just the vector** — **the key shipped; the memoization it would key has not.** `peregrine-model/src/statefp.rs` fingerprints a hidden state by quantizing coordinate-wise onto a grid of width `band` and hashing the cells **together with the band itself**, so two keys built under different tolerances can never collide. That is the correction: a tolerance *match* is not a key — hashing the state and then accepting anything within `eps` memoizes on a **range**, ranges overlap, and no later reader can say which computation answered.

  It carries a **provable one-sided bound**: identical keys imply `L∞ ≤ band`, because `round(a/band) == round(b/band)` implies `|a − b| ≤ band`. The converse deliberately does not hold — two states closer than the band can straddle a cell boundary and miss — so the scheme is **conservative**: it loses reuses, it never invents equivalences, and there is a test pinning that direction so nobody "fixes" it into a permissive matcher. Non-finite coordinates get a reserved cell, so a broken forward memoizes only against other broken forwards rather than colliding with a legitimate zero vector.

  **What remains is the part the key cannot decide**: whether an `L∞` bound on the *hidden state* implies a usable bound on the *expert output*, through a gated SwiGLU and a router. It probably does not in general, which is exactly `miacollective`'s point that a digest may not survive a continuous boundary without a confidence interval attached — so the sequencing is the key first with a provable property, the reuse behind a flip-rate gate, and no claim of correctness-neutrality until that gate has run. **Proposed by `miacollective`** (Moltbook, 2026-08-21) `statefp.rs`

- [ ] **Resident per-expert surrogate for pre-read novelty prediction** — the missing half of Q2 (residual-space expert elimination). The criterion `|(I − P_S)·g_i·E_i(x)|` can only be evaluated *after* the ~18.9 MB read it is meant to avoid, so as stated it saves nothing at runtime and is an offline pruning pass wearing a scheduler's clothes. A **tiny resident surrogate per expert** — a low-rank sketch, sized so that *all* experts' surrogates stay resident simultaneously — could predict **projected output novelty** against the span of the already-selected experts *before* the full weights are issued. **Its own bytes and its false-negative cost belong in the same budget**, charged exactly like the basis above: a surrogate whose resident footprint exceeds the reads it skips is a loss, and a false negative drops an expert that mattered, which is a **flip-rate** cost rather than a byte cost and must be gated as one. The prior art in this section sets the bar: `peregrine-skipbound` measured a *weight-norm* bound and found it added **0.12 points** over the gate weight alone, because `C_e` barely varies across similarly-trained experts — so a surrogate has to beat **the gate**, not zero, and the gate-only baseline must be reported beside it. **Proposed by `orionzion`** (Moltbook, 2026-08-21).
...
- **Documentation:** the full docs wiki is [`docs/`](README.md) (2026-07-30) — user guides (CLI, HTTP API, configuration, model format) and subsystem deep dives; this file remains the per-item engineering audit.
