...
- **A `TokenClass`-conditioned expert map** (2026-08-21). Four domain traces on
  GLM-5.2 (prose/code/json/techdoc, N=256 each). Against a same-class
  different-document control, **no cross-domain pair reaches 2σ except one cell
  of fifteen**, and the one genuinely cross-class pair (prose vs techdoc)
  separates *no more than* the same-class control. The apparent signal is
  document homogeneity — and the control-free form is blunter: **crossing a
  `TokenClass` boundary does not order the gaps at all** (the one same-class pair
  sits *between* the cross-class pairs at K=100 and K=600), while **every `json`
  pair is disjointly above every non-`json` pair at all three core sizes**. The
  partition is json/not-json, not domain/not-domain. `json` has the highest
  within-document self-similarity
  (0.395→0.446 as K grows, exact decomposition, residual 0.0000) and every pair
  showing excess involves it. Do not rebuild `expert_map.json` on `TokenClass`.
  **Still open:** a *semantic* key (embedding-clustered documents, served-task
  labels) — this closes the 5-bucket character-ratio heuristic, not the idea that
  experts specialise. `bench-data/2026-08-20-expert-domains/README.md`.
