...
| sPTC speculative tool calling | `peregrine-serve` (`sptc.rs`, `tools.rs`) | ✅ | hosted tools (`--host-tool`, closed pure set: tokenize/detokenize/count_tokens) executed while the model is still streaming the call — pure-by-construction decorator (`pure_tool!` is the only `HostedTool` constructor), exact-canonical-key verification (a wrong guess is a discard, never a wrong answer), bounded shadow executor, `peregrine_result` wire extension on hosted calls only, `COLI_SPTC=0` kill switch, counters on `/metrics` — see [docs/sptc.md](docs/sptc.md) |
...
| `COLI_SSD_AWARE_SCHED` | off | Learned SSD clock — per-device bandwidth EWMA pricing claim windows in predicted time; shortest-job-first order inside each device group and ring homes weighted by seconds (slow device earns more rings). Reorder only: output bit-identical. Neutral until the clock has measured a spread |
| `COLI_ROUTER_LOOKAHEAD_K` | 1 | Multi-layer router look-ahead — reach k layers ahead, each deeper horizon spending half the window (`6→3→1→0`). Advisory prefetch only: cannot change a token. Horizon 1 is the historical behaviour |
