...
impl LayerKv {
    ...
    fn append(&mut self, pos: usize, lc_row: &[f32], rc_row: &[f32]) {
        // Engine-internal invariant: positions are generated by the forward
        // loop, never taken from network/stdio input. An out-of-order append
        // would silently corrupt attention for the rest of the sequence, so a
        // loud stop beats degraded output — the one deliberate production
        // assert in the workspace.
        assert_eq!(pos, self.len, "KV cache must be appended in position order"); // audit-allow: internal invariant
